...
public class DynamicFilmItem extends FilmRollItem implements SensitiveFilmItem {
    ...
    public DynamicFilmItem(ExposureType type, int barColor, Properties properties,
        ...
        super(type, barColor, properties, FilmStyle.EMPTY);
    ...
    public int getMaxFrames(ItemStack stack) {
        return DynamicPhotographValidation.validateMaxFrames(
                DynamicItemStackData.getDynamicFilmMaxFrames(stack, defaultMaxFramesSupplier.getAsInt()));
    }

    public int getUsedFrames(ItemStack stack) {
        int totalFrames = getMaxFrames(stack);
        int usedFrames = DynamicItemStackData.getDynamicFilmUsedFrames(stack, 0);
    ...
    public void setRecordedPhotograph(ItemStack stack, DynamicPhotographCreationData creationData) {
        DynamicItemStackData.setDynamicPhotographFrames(stack, creationData.frames());
        DynamicItemStackData.setDynamicPhotographSettings(stack, creationData.settings());
        DynamicItemStackData.setDynamicPhotographSummary(stack, creationData.summary());
        DynamicItemStackData.setDynamicPhotographSessionId(stack, creationData.sessionId());
        DynamicItemStackData.setDynamicFilmUsedFrames(stack, creationData.summary().frameCount());
    ...
    public List<Frame> getStoredFrames(ItemStack stack) {
        DynamicPhotographFrames storedFrames = DynamicItemStackData.getDynamicPhotographFrames(stack, DynamicPhotographFrames.EMPTY);
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        ...
        tooltipComponents.add(Component.translatable("item.exposure_camcorder.dynamic_film.tooltip.remaining_frames", remainingFrames)
                .withStyle(ChatFormatting.GRAY));
        tooltipComponents.add(Component.translatable("item.exposure_camcorder.dynamic_film.tooltip.duration", remainingFrames)
                .withStyle(ChatFormatting.GRAY));
    }
    ...
    public static void syncExposureCompatibilityComponents(ItemStack stack, int actualFrameCount, int frameSize) {
        ...
        io.github.mortuusars.exposure.Exposure.DataComponents.setFilmFrames(stack,
                compatFrameCount > 0 ? Collections.nCopies(compatFrameCount, Frame.EMPTY) : List.of());
        io.github.mortuusars.exposure.Exposure.DataComponents.setFilmFrameSize(stack, frameSize);
    }
}
