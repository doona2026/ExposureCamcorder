...
public class DynamicRecordingStatusOverlay {
    ...
    public boolean shouldRender(DynamicCaptureSession session) {
    ...
    public int remainingDurationTicks(DynamicCaptureSession session, long currentTick) {
        if (session == null) {
            return 0;
        }

        long remaining = (long) session.maxRecordingDurationTicks() - session.elapsedTicks(currentTick);
        return (int) Math.max(0L, remaining);
    }

    public double remainingDurationSeconds(DynamicCaptureSession session, long currentTick) {
        return DynamicPhotographTiming.ticksToSeconds(remainingDurationTicks(session, currentTick));
    }

    public int remainingCapacity(DynamicCaptureSession session) {
        if (session == null) {
            return 0;
        }

        return Math.max(0, session.maxFrames() - session.frameCount());
    }

    public List<Component> createStatusLines(DynamicCaptureSession session, long currentTick) {
        return List.of(
                Component.translatable("gui.exposure_camcorder.recording_status.rec"),
                Component.translatable("gui.exposure_camcorder.recording_status.frames", recordedFrames(session)),
                Component.translatable("gui.exposure_camcorder.recording_status.remaining_duration",
                        DynamicPhotographTiming.formatSeconds(remainingDurationSeconds(session, currentTick))),
                Component.translatable("gui.exposure_camcorder.recording_status.remaining_capacity",
                        remainingCapacity(session))
        );
    }

    public int recordedFrames(DynamicCaptureSession session) {
        if (session == null) {
            return 0;
        }
    ...
    public double remainingDurationSeconds(DynamicRecordingClientState state) {
        return state == null ? 0d : DynamicPhotographTiming.ticksToSeconds(state.remainingDurationTicks());
    }
    ...
    public float progressFraction(DynamicRecordingClientState state) {
        ...
        return Math.clamp((float) state.recordedFrames() / (float) state.maxFrames(), 0f, 1f);
    }

    public void render(GuiGraphics guiGraphics, Font font, int x, int y, DynamicCaptureSession session, long currentTick) {
        if (!shouldRender(session)) {
            return;
        }

        List<Component> lines = createStatusLines(session, currentTick);
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(x, y, 0);
        guiGraphics.pose().scale(HUD_SCALE, HUD_SCALE, 1f);
        renderStatusLines(guiGraphics, font, lines);
